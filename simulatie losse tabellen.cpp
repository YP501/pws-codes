...
int main()
{
  ...
  for (int gamesPlayed = 1; gamesPlayed < totalGames + 1; gamesPlayed++)
  {
    ...
    // Per game wordt er een CSV bestand gemaakt
    ofstream csvResults("./CSVs/gameData" + to_string(gamesPlayed) + ".csv");
    ...
    for (int round = 0; round < cardGame.initialDeckSize; round++)
    {
      ...
      // Schrijf ronde resulaten naar het CSV bestand voor deze ronde
      csvResults << cardGame.getRoundDataRaw();

      cardGame.endRound();
    };
    // Sluit het CSV bestand af en sla het op
    csvResults.close();
  }
